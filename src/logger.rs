...
impl Logger {
    ...
    pub fn get_entries(&self,start_index:usize,end_index:usize,filter:Verbosity)->  Result<Vec<(u32,String,Verbosity)>,String>{
        let pool = self.pool.lock().map_err(|_| "pool lock failed!".to_string())?;
        let sub_pool = pool.range(start_index..end_index).filter(|log| log.2 <= filter).cloned().collect();
        Ok(sub_pool)
    }
    pub fn print_log(&self){
        todo!();
    }
    pub fn print_log_level(&self,predicator:Verbosity) {
        todo!()
    }
}